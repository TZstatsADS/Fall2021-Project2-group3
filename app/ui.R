    ...
    navlistPanel(
        ...
        tabPanel("Cases by Borough",
                 ...
                 p("This section offers information about the confirmed, probable,
                   hospitalized and death cases caused by COVID-19 in 2021. You 
                   can use the range selector under each time series plot to 
                   have a closer look at any time period you want.")),
        
        "Reopening Data Now",
        tabPanel("Outdoor storefronts", 
                 h4(strong("Check out all storefronts who want to use outdoor areas on 
                 a temporary bases and take care!")),
        ...
        tabPanel("DHS Shelter System",
                 h4("Shelter Data"),
                 p(strong("How many people need assistance from the Department 
                          of Homeless Services since reopening?)")),
                 ...
                 p("Looking at the data from 2020 onwards, we can see that there 
                   ...
                   residing in the Department of Homeless Services in July of 
                   this year, though the number has increased slightly since then."),
                 p("This dip in reported homeless peope correates well with the 
                   low number of covid cases during that time of July. Meaning 
                   that there might be a link between the two. Drop in covid cases
                   occuring at the same time at drop in reported homeless people."),
                 br(),
                 p(strong("Where can individuals go for help, and what resources
                          are available to them from the Department of Homeless 
                          Services?")),
                 leafletOutput("leafmap_1"),
                 p("This map shows the location of homebase locations for the 
                   Department of Homeless Services (DHS), as well as the number 
                   of shelter buildings available at each of these community 
                   districts.")),
        "----------",
        tabPanel("About Project",
                 h4("Reopen NYC Report"),
                 p("Reopen NYC Report is a tool which provides daily COVID-19 
                   update and useful information about NYC reopening. 
                   As project develops, more information about reopening such as
                   storefront schedule, DHS Shelter System policy will be added."),
                 br(),
                 h4("Precaution"),
        ...
        tabPanel("Conclusions",
                 h4("Here is a write-up of the conclusions we observed from 
                    our data analysis and analysis of the interactive graphs 
                    provided."),
                 p("Through looking at the NYC data on COVID-19 cases, represented 
                   by a gif in the first tab of this shiny app, we can see that 
                   while the death rate and hospitalization rates are now quite 
                   low, COVID-19 is still spreading in New York City, with a 
                   fluctation between 500 and 2000 cases daily. This should be 
                   a reminder that while the pandemic is less dangerous than it 
                   used to be, it is still something to be concerned about in New York City."),
                 br(),
                 p("Looking at the data from 2020 onwards, we can see that there 
                   has been a general decrease in the number of people who have 
                   been reported to be homeless. There was a drop in the number 
                   people residing in the Department of Homeless Services in July
                   of this year, though the number has increased slightly since 
                   then."),
                 br(),
                 p("This dip in reported homeless peope correates well with the 
                   low number of covid cases during that time of July. Meaning 
                   ...
                   occuring at the same time at drop in reported homeless people"),
                 br(),
                 p("Storefronts are adapting to Covid by using sidewalk seating 
                   etc. As shown by the interactive map, there are quite a few 
                   stores in New York City, reopening like so."))
    )
